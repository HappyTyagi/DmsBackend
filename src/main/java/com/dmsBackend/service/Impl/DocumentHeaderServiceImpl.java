...
//    }
    ...
    public List<DocumentHeader> findAllRejectedByActionEmployeeId(Integer employeeId) {
        ...
        // Check the employee's role to determine document access
        if (employee.getRole().getRole().equalsIgnoreCase("ADMIN")) {
            // If Admin, fetch all rejected documents
            return documentHeaderRepository.findAllByApprovalStatus(DocApprovalStatus.REJECTED);
        } else if (employee.getRole().getRole().equalsIgnoreCase("BRANCH ADMIN")) {
            // If Branch Admin, fetch rejected documents in the branch
            return documentHeaderRepository.findAllRejectedByBranch(employee.getBranch(), DocApprovalStatus.REJECTED);
        } else if (employee.getRole().getRole().equalsIgnoreCase("DEPARTMENT ADMIN")) {
            // If Department Admin, fetch rejected documents in the department
            return documentHeaderRepository.findAllRejectedByDepartment(employee.getDepartment(), DocApprovalStatus.REJECTED);
        } else {
    ...
    public List<DocumentHeader> findAllApprovedByActionEmployeeId(Integer employeeId) {
        ...
        if ("ADMIN".equalsIgnoreCase(employee.getRole().getRole())) {
            return documentHeaderRepository.findAllByApprovalStatus(DocApprovalStatus.APPROVED);
        } else if ("BRANCH ADMIN".equalsIgnoreCase(employee.getRole().getRole())) {
            return documentHeaderRepository.findAllApprovedInBranch(employee.getBranch(), DocApprovalStatus.APPROVED);
        } else if ("DEPARTMENT ADMIN".equalsIgnoreCase(employee.getRole().getRole())) {
            return documentHeaderRepository.findAllApprovedInDepartment(employee.getDepartment(), DocApprovalStatus.APPROVED);
        } else {
    ...
    public List<DocumentHeader> getPendingDocumentsByBranch(Integer branchId) {
        return documentHeaderRepository.findPendingByBranch(branchId);
    ...
    public List<DocumentHeader> findApprovedDocumentsByBranchAdmin(Integer employeeId) {
        ...
        return documentHeaderRepository.findByEmployeeByIdAndApprovalStatus(employeeId, approvalStatus);
    }

    @Override
    public List<DocumentHeader> getAllPendingDocuments() {
        // Pass the enum value here
        return documentHeaderRepository.findAllByApprovalStatus(DocApprovalStatus.PENDING);
    }

//    @Override
//    public Long countDocumentHeadersByBranchId(Integer branch) {
//        return documentHeaderRepository.countByEmployee_BranchId(branch);
//    }
    ...
    public Long countPendingDocumentsByBranchId(Integer branch) {
        return documentHeaderRepository.countByEmployee_BranchIdAndApprovalStatus(branch, DocApprovalStatus.PENDING);
    }

    @Override
    public Long countApprovedByBranchId(Integer branch) {
        return documentHeaderRepository.countByEmployee_BranchIdAndApprovalStatus(branch, DocApprovalStatus.APPROVED);
    }

    @Override
    public Long countRejectedByBranchId(Integer branch) {
        return documentHeaderRepository.countByEmployee_BranchIdAndApprovalStatus(branch, DocApprovalStatus.REJECTED);
    }
